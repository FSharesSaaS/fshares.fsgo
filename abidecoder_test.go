...
func TestABI_DecodeMissingData(t *testing.T) {
	...
	assert.Equal(t, fmt.Errorf("read: varint: invalid buffer size"), err)
...
func TestABI_decodeFieldsErr(t *testing.T) {
	...
	assert.Equal(t, fmt.Errorf("read field of type [bad.type.1]: unknown type"), err)
...
func TestABI_Read(t *testing.T) {
	...
	for _, c := range tests {
		t.Run(c.name, func(t *testing.T) {
			...
			json, err := abi.read(NewDecoder(buffer.Bytes()), c.fieldName, c.typeName, []byte{})
			if c.expectError {
				assert.Error(t, err)
			} else {
				require.NoError(t, err)
				assert.Equal(t, value, gjson.GetBytes(json, c.fieldName).Raw, c.name)
			}
		})
	}
}
...
func TestABI_Read_Symbol(t *testing.T) {
	...
	out, err := abi.read(NewDecoder(data), "name", "symbol", []byte("{}"))
	require.NoError(t, err)
...
func TestABI_Read_TimePointSec(t *testing.T) {
	...
	require.NoError(t, err)
	out, err := abi.read(NewDecoder(data), "name", "time_point_sec", []byte("{}"))
...
func TestABI_Read_SymbolCode(t *testing.T) {
	...
	out, err := abi.read(NewDecoder(data), "name", "symbol_code", []byte("{}"))
	require.NoError(t, err)
